...
tokensRouter.get("/", (req, res) => {
  ...
  if (chain === "stellar") {
    return res.json({ tokens: STELLAR_TOKENS, chain: "stellar" });
  }
  if (chain && chain in SUPPORTED_TOKENS) {
    return res.json({
      tokens: SUPPORTED_TOKENS[chain as keyof typeof SUPPORTED_TOKENS],
      chain,
    });
  }
  res.json({ tokens: SUPPORTED_TOKENS, stellarTokens: STELLAR_TOKENS });
});

// ─── GET /tokens/stellar ────────────────────────────────────────────────────────

tokensRouter.get("/stellar", (_req, res) => {
  res.json({ tokens: STELLAR_TOKENS });
});
